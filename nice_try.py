...
# 克隆自聚宽文章：https://www.joinquant.com/post/67584
# 标题：五合一打板策略我去掉了同花顺获取，收益上升一点，回撤小了一点
# 作者：好玩的豆豆
...
def get_yesterday_limitup_stocks(date):
    """
    获取指定日期（昨日）的所有涨停股票
    参数:
...
def get_concept(stock_list, date):
    """
    获取股票列表所属的聚宽概念
    返回格式：{stock: [{'concept_name': name, 'concept_code': code}, ...]}
    """
    result = {}
    for stock in stock_list:
        try:
            info = get_security_info(stock)
            if info and hasattr(info, 'concepts') and info.concepts:
                concepts = []
                for c in info.concepts:
                    # 聚宽概念数据格式：每个概念是一个字典，包含 'name' 和 'code'
                    if isinstance(c, dict):
                        concepts.append({'concept_name': c.get('name', ''), 'concept_code': c.get('code', '')})
                result[stock] = concepts
            else:
                result[stock] = []
        except Exception as e:
            log.warning(f"获取股票 {stock} 概念失败: {str(e)}")
            result[stock] = []
    return result


def get_hot_concepts_from_limitup(context, top_n=8):
    """
    根据昨日涨停股票的概念分布，统计热门概念（按涨停股数排序）
    参数：
        context: 上下文对象
        top_n: 返回的热门概念数量，默认8个
    返回：
        list: 概念名称列表
    """
    date_str = context.previous_date.strftime('%Y-%m-%d')
    # 检查缓存
    if hasattr(g, 'hot_limitup_cache') and date_str in g.hot_limitup_cache:
        return g.hot_limitup_cache[date_str]

    # 获取昨日涨停股
    limitup_stocks = get_yesterday_limitup_stocks(date_str)
    if not limitup_stocks:
        log.warning("昨日无涨停股票，无法统计热点概念")
        return []

    # 获取涨停股的概念分布
    concept_dict = get_concept(limitup_stocks, date_str)
    ...
    # 按涨停股数排序，取前 top_n
    sorted_concepts = sorted(concept_count.items(), key=lambda x: x[1], reverse=True)[:top_n]
    hot_names = [name for (code, name), count in sorted_concepts]

    # 存入缓存
    g.hot_limitup_cache[date_str] = hot_names
    log.info(f"本地热点概念统计（基于昨日{len(limitup_stocks)}只涨停股）: {hot_names}")
    return hot_names
...
def check_cache_status():
    ...
    """
    try:
        if not hasattr(g, 'hot_limitup_cache'):
            return "未初始化"
        cache_count = len(g.hot_limitup_cache)
        return f"本地缓存正常({cache_count}个交易日)"
    except Exception as e:
        return f"检查失败: {str(e)}"
...
def calculate_mainline_score_optimized(stock, context):
    ...
    try:
        # 获取热门概念列表
        hot_concepts = get_hot_concepts_from_limitup(context, top_n=g.concept_num)
        hot_concepts_set = set(hot_concepts)

        if not hot_concepts_set:
            log.debug(f"热点概念列表为空，主线评分为0")
            return 0

        # 获取股票所属概念
        stock_info = get_security_info(stock)
        if not stock_info or not hasattr(stock_info, 'concepts') or not stock_info.concepts:
            log.debug(f"股票 {stock} 无所属概念")
            return 0

        # 从概念字典中提取'name'字段，得到字符串列表
        stock_concepts = [concept['name'] for concept in stock_info.concepts if isinstance(concept, dict)]
        ...
        if match_count > 0:
            log.info(f"股票 {stock} 匹配到热门概念（去重后）: {unique_matched}, 匹配数量: {match_count}, 主线评分: {mainline_score}")
        else:
            log.debug(f"股票 {stock} 未匹配到热门概念，主线评分: {mainline_score}")
